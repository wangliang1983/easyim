...
public interface IMessageMapper {
	...
	@Insert("insert into t_message (id,tenement_id,product,from_id,to_id,proxy_from_id,proxy_to_id,cid,proxy_cid,type,sub_type,content,biz_uid,gmt_create) values "
								+ "(#{m.id},#{m.tenementId},#{m.product},#{m.fromId},#{m.toId},#{m.proxyFromId},#{m.proxyToId},#{m.cid},#{m.proxyCid},#{m.type},#{m.subType},#{m.content},#{m.bizUid},#{m.gmtCreate})")
